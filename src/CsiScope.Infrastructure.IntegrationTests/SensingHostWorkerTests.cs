...
public class SensingHostWorkerTests
{
    ...
    public async Task Csi_frame_flows_through_to_orchestrator_baseline()
    {
        ...
        await harness.WriteLineAsync(CsiLine);
    ...
    public async Task Command_egress_writes_set_rf_and_ack_completes_quorum()
    {
        ...
        var commandJson = await harness.ReadLineAsync(TimeSpan.FromSeconds(2));
    ...
    [Fact]
    public async Task Torn_and_crlf_frames_still_parse()
    {
        // Arrange
        await using var harness = new WorkerTestHarness();
        await harness.StartAsync();

        // Act — split the frame mid-line, then finish with CRLF.
        var split = CsiLine.Length / 2;
        await harness.WriteRawAsync(CsiLine[..split]);            // no newline — partial
        await Task.Delay(50);                                     // separate read chunk
        await harness.WriteRawAsync(CsiLine[split..] + "\r\n");

        // Assert — framing reassembled the line; baseline exists.
        await WorkerTestHarness.WaitForAsync(() => harness.Orchestrator.TrackedBaselineCount == 1);
    }
}
